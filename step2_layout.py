...
def compact_block(block: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "type": block.get("type"),
        "text": text_excerpt(text, limit=300),
    }
...
def build_packets(
    ...
    for page in page_numbers(blocks):
        ...
        packet = compact_page_packet_namespaced(run_id, doc_dir, out_run_dir, page, namespace, blocks, labels)
...
def build_range_messages(packets: list[dict[str, Any]], run_id: str, stream: str, mode: str) -> list[dict[str, Any]]:
    compact = {
        "run_id": run_id,
        "stream": stream,
        "mode": mode,
    ...
    instruction = (
        ...
        "只返回严格 JSON，格式如下：\n"
        '{"question_ranges":[{"question_no":1,"start_label":"Q-V01-B01","end_label":"Q-V01-B03","visual_labels":["Q-V01-P01"],"confidence":0.0,"reason":"简短理由"}],'
        '"noise_blocks":["Q-V01-B01"],'
        '"risks":[{"type":"boundary|missing|uncertain","question_no":1,"block_labels":["Q-V01-B02"],"severity":"info|warning|error","evidence":"简短证据"}]}\n\n'
        "紧凑版面包：\n"
    ...
    return [
        {"role": "system", "content": "只检测试卷 OCR 顶层题号的起止块范围。返回严格 JSON。"},
...
def call_range_detector(
    ...
    messages = build_range_messages(packets, run_id, stream, mode)
    parsed, raw, elapsed = call_chat_json(messages, model=model, timeout=timeout)
...
def build_alignment(
    ...
    if pipeline_mode == "paper_plus_answer_file":
        ...
        for item in (question_ranges or {}).get("question_ranges") or []:
            ...
            q_labels = labels_for_range(item, question_packets)
            a_labels = answer_range_by_qno.get(qno, [])
            q_surface_labels = question_surface_labels_from_full_range(q_labels, question_blocks)
            assets = rule_visual_assets(q_labels, question_blocks)
        ...
        for item in (mixed_ranges or {}).get("question_ranges") or []:
            ...
            q_labels = [label_to_q(label) for label in mixed_labels]
            q_labels = [label for label in q_labels if label in question_blocks]
            q_surface_labels = question_surface_labels_from_full_range(q_labels, question_blocks)
            assets = rule_visual_assets(q_labels, question_blocks)
            question_groups.append(
                {
...
def run_one(
    ...
    question_packets = build_packets(run_id, paper_dir, out_run_dir, paper_blocks, "Q", "question_packets")
    ...
    if pipeline_mode == "paper_plus_answer_file":
        ...
        answer_packets = build_packets(run_id, answer_dir, out_run_dir, answer_blocks, "A", "answer_packets")
        tasks = {
            "pure_paper": {
        ...
        answer_packets = build_packets(run_id, paper_dir, out_run_dir, paper_blocks, "A", "answer_packets")
