...
def render_blocks(blocks: Any, run_id: str, record: dict[str, Any], asset_root: Path, out_path: Path, option_key: str | None = None) -> str:
    ...
    if option_key:
        ...
        if isinstance(asset, dict) and asset.get("placeholder") and asset.get("label"):
            replacements[str(asset["placeholder"])] = render_asset(run_id, str(asset["label"]), asset_root, out_path, inline=True)
    for block in as_blocks(blocks):
        rendered = render_block(block)
        for placeholder, replacement in replacements.items():
            rendered = rendered.replace(esc(placeholder), replacement)
...
def render_options(run_id: str, record: dict[str, Any], asset_root: Path, out_path: Path) -> str:
    ...
    for key in option_keys:
        ...
        if asset_label and (not placeholder or str(placeholder) not in as_blocks(value)):
