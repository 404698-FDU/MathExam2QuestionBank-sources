...
本目录保存 Step2-Step5 与大模型交互相关的运行时 prompt，以及仍参与当前设计审阅的算法说明。

当前状态：

- `*.prompt.md` 是 v12 runtime 实际读取的轻量 prompt 文件，固定使用 `meta/system/user` 三段结构。
- 其余 `*.md` 是当前仍需随 prompt 审阅的算法和流转说明，不直接作为运行时 prompt 源。
- 旧的长版 prompt 契约文档已迁移到 `../../doc/zh_CN/`，只作为历史设计证据，不再作为 `check_contracts.py` 的机器校验目标。
- v12 运行代码必须从本目录加载 prompt，不得再内嵌大段 prompt。
- 后续实现 `exam_import.prompts` 时，必须以本目录为 prompt source of truth。
- prompt、tool schema、运行时 schema 的分层放置规则见上级文档 `assets/v12_runtime/V12_PROMPT_SCHEMA_LAYOUT.zh-CN.md`。
