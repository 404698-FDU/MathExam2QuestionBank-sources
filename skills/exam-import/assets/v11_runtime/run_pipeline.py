...
def resolve_pipeline_paths(args: argparse.Namespace) -> None:
    args.source_runs_root = args.source_runs_root.resolve()
    args.runs_root = args.runs_root.resolve()
    args.qb_root = args.qb_root.resolve()
    args.render_root = args.render_root.resolve()
    args.step3_5_root = args.step3_5_root.resolve()
    if args.token_budget_log is not None:
        args.token_budget_log = args.token_budget_log.resolve()
    if args.token_budget_pool is not None:
        args.token_budget_pool = args.token_budget_pool.resolve()


def validate_step3_inputs(run_ids: list[str], runs_root: Path) -> None:
    ...
    for run_id in run_ids:
        ...
        nearby = sorted(
            str(path.resolve())
            for path in runs_root.glob("*_raw_units/qa_alignment.json")
            if path.is_file()
        )[:10]
        missing.append(
            {
                "run_id": run_id,
                "expected": str(alignment_path.resolve()),
                "nearby_qa_alignment": nearby,
            }
        )
    if missing:
        raise FileNotFoundError(
            "Step3 input preflight failed; missing Step2 qa_alignment.json. "
            f"runs_root={runs_root.resolve()} details={json.dumps(missing, ensure_ascii=False)}"
        )
...
def main() -> int:
    ...
    parser.add_argument("--llm-provider", choices=["siliconflow", "bailian", "bailian_batch", "env"], default="siliconflow")
    ...
    parser.add_argument("--step3-structured-output", choices=["json_schema", "tool_calling"], default="tool_calling")
    parser.add_argument("--step3-no-ocr-input", action="store_true")
    parser.add_argument("--step2-structured-output", choices=["json_object", "tool_calling"], default="tool_calling")
    parser.add_argument("--step3-5-structured-output", choices=["json_schema", "tool_calling"], default="tool_calling")
    parser.add_argument("--step4-structured-output", choices=["json_object", "tool_calling"], default="tool_calling")
    args = parser.parse_args()
    resolve_pipeline_paths(args)
    ...
    if not args.skip_step2:
        for run_id in args.run_id:
            ...
            summary = step2_layout.run_one(
                ...
                mode=args.mode,
                worker_models=worker_models,
                structured_output=args.step2_structured_output,
    ...
    token_budget_summary = {
        ...
        "worker_models": worker_models,
        "structured_outputs": {
            "step2": args.step2_structured_output,
            "step3": args.step3_structured_output,
            "step3_5": args.step3_5_structured_output,
            "step4": args.step4_structured_output,
        },
        "step3_no_ocr_input": args.step3_no_ocr_input,
        "log_path": str(token_log_path) if token_budget_enabled else None,
    }
    write_json(args.runs_root / "batch_summary.json", {"runs": summaries, "model": args.model, "token_budget": token_budget_summary})

    if not args.skip_step3:
        validate_step3_inputs(args.run_id, args.runs_root)
        if len(worker_models) > 1:
            ...
            run_step3_multi_model(
                ...
                force=args.force,
                enable_thinking=args.enable_thinking,
                structured_output=args.step3_structured_output,
                no_ocr_input=args.step3_no_ocr_input,
                env=env,
            )
        else:
    ...
    if args.step3_5:
        ...
        run_step3_5(
            ...
            max_workers=args.max_workers,
            enable_thinking=args.enable_thinking,
            structured_output=args.step3_5_structured_output,
            env=env,
        )
    ...
    if not args.skip_step4:
        ...
        for run_id in args.run_id:
            visual_summary = step4_asset_distribution.assign_visual_assets_for_run(
                ...
                force=args.force,
                worker_models=worker_models,
                structured_output=args.step4_structured_output,
                enable_thinking=args.enable_thinking,
            )
