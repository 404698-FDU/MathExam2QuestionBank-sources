...
STEP3_IMAGE_ONLY_TOOL_SCHEMA = copy.deepcopy(STEP3_TOOL_SCHEMA)
STEP3_IMAGE_ONLY_TOOL_SCHEMA["properties"]["issues"]["items"]["properties"]["type"] = {
    "type": "string",
    "enum": [
        "image_unclear",
        "boundary_suspect",
...
def normalize_double_escaped_latex(text: str) -> str:
    text = DOUBLE_ESCAPED_LATEX_RE.sub(r"\\", text)
    if re.search(r"\\begin\{(?:aligned|cases|matrix|pmatrix|bmatrix|array)\}", text):
        text = re.sub(r"\\{4,}(?=\s)", lambda _match: r"\\", text)
    return text
...
def parse_stringified_array(value: Any) -> list[str] | None:
...
def normalize_stringified_array_fields(record: dict[str, Any]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    out = dict(record)
    repairs: list[dict[str, Any]] = []
    for field_name in ("stem_latex", "answer_latex", "analysis_latex", "rubric_latex"):
        parsed = parse_stringified_array(out.get(field_name))
        if parsed is not None:
            out[field_name] = parsed
            repairs.append(
                {
                    "kind": "stringified_array_to_array",
                    "field": field_name,
...
def normalize_grouped_choice_options(record: dict[str, Any]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    out = dict(record)
    repairs: list[dict[str, Any]] = []
    question_type = str(out.get("question_type") or "")
    options = out.get("options_latex")
    if question_type not in {"single_choice", "multiple_choice"} or not isinstance(options, dict):
        return out, repairs
    normalized = normalize_options_latex(options)
    a_items = list(normalized.get("A") or [])
    b_items = list(normalized.get("B") or [])
    c_items = list(normalized.get("C") or [])
    d_items = list(normalized.get("D") or [])
    if len(a_items) == 2 and len(b_items) == 2 and not c_items and not d_items:
        out["options_latex"] = {
            "A": [a_items[0]],
            "B": [a_items[1]],
            "C": [b_items[0]],
        ...
        repairs.append(
            {
                "kind": "grouped_choice_options_to_abcd",
                "field": "options_latex",
                "pattern": "A2_B2_C0_D0",
                "item_count": 4,
            }
...
def normalize_empty_latex_array_items(record: dict[str, Any]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    out = dict(record)
    repairs: list[dict[str, Any]] = []
    for field_name in ("stem_latex", "answer_latex", "analysis_latex", "rubric_latex"):
        ...
        if removed:
            ...
            repairs.append(
                {
                    "kind": "remove_empty_latex_array_items",
                    "field": field_name,
...
def normalize_structural_fields(record: dict[str, Any]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    out, repairs = normalize_stringified_array_fields(record)
    out, empty_repairs = normalize_empty_latex_array_items(out)
    repairs.extend(empty_repairs)
    out, option_repairs = normalize_grouped_choice_options(out)
    repairs.extend(option_repairs)
    return out, repairs
...
def build_question_payload(
    ...
    question_core_labels = unique_str(list(row.get("question_core_labels") or []))
    ...
    return {
        ...
        "question_core_labels": question_core_labels,
        ...
        "source_question_core_labels": question_core_labels,
...
如果答案/解析裁剪图只给出最终答案、答案表或答案列表，没有可见解析步骤，则 analysis_latex 必须为空数组 []，不得自行推导解析。
如果图片中看不清、边界不确定、答案未独立给出，必须在工具参数 issues 中记录。
"""

IMAGE_ONLY_USER_PROMPT_TEMPLATE = """\
...
- 如果 A/B/C/D 四个选项横排在同一行、同一个文字块或同一张裁剪图中，也必须按可见选项标签分别写入 A、B、C、D；不得把多个选项合并到 A 或任意一个键中。
- 标准 A/B/C/D 选择题中，每个键通常只对应一个可见选项；不得按行、列、左右栏或视觉分组把两个选项放进同一个键。例如两行两列排版时，仍按标签分配为 A、B、C、D，而不是把左列放 A、右列放 B。
- 非选择题四个键都填空数组。
- 如果某个选项只有图，没有文字，应把对应图片标号写入该选项数组，例如 ["Q-V02-P01"] 或 ["M-V02-P01"]；不要输出 <image01> 这类临时占位符，不要描述图片内容。

5. answer_latex
- 只填写答案图中明确独立给出的最终答案、答案行或各小题答案；不要放证明过程、计算展开和最大值/分类讨论的理由。
- 不要从解析过程中反推答案。
- 数学答案必须用 $...$。
- 选择题答案字母必须写成完整数学块，例如 $A$、$B$、$C$、$D$，不得写成缺少右定界符的 $A。
- 如果没有独立答案行，填空数组，并在 issues 记录 answer_missing。

6. analysis_latex
- 填答案/解析图中的解答步骤、证明过程、计算过程、思路分析、点评。
- 如果答案/解析图只给出最终答案、答案表或答案列表，没有可见解答步骤、证明过程、计算过程、思路分析、解析、点评，则 analysis_latex 必须输出空数组 []。
- 不得根据题面和答案自行推导、补写或生成解析；analysis_latex 只能来自答案/解析图中实际可见的解析内容。
- 答案/解析图中每一行有效数学推导、比较、分类讨论、验证、结论理由都必须覆盖到 analysis_latex。
- 不要摘要，不要改写，不要删减；不得把完整解析压缩成“核心步骤 + 最终结论”。
- 即使某一步看起来只是补充分数、最大值比较、是否超过阈值、分类区间验证，也必须保留。
- 答案/解析裁剪图中每一个可见的有效文本块都要在 answer_latex 或 analysis_latex 中出现；其中解题过程、比较过程、证明收尾必须放入 analysis_latex。
- 即使同一句结论已经作为最终答案写入 answer_latex，只要它在答案/解析图中作为推导收尾、比较判断或理由出现，也仍然必须保留在 analysis_latex。
- 对含分段函数、最值、范围、是否超过阈值的问题，必须保留图中最后的分段汇总、最大/最小值比较和最终判断理由。
- 输出前逐项核对答案/解析裁剪图中的可见解析块；若最后的判断、比较、分类讨论、证明收尾缺失，视为错误输出。
- 如果解析看不清，保留能确认的内容，并在 issues 记录 image_unclear。
...
- 如果图片中一段内容跨多行，但语义仍是一段，转写为一个单行字符串，中间用正常标点连接，不保留真实换行。
- 如果图片中是新的小问、新的解答步骤、新的自然段、【分析】、【解答】、【点评】等段落，则拆成新的数组元素。
- 每个数组元素必须是单行 JSON 字符串，禁止包含真实换行或 \n。
- stem_latex、answer_latex、analysis_latex、rubric_latex 必须是真正的 JSON 数组，不能把整个数组写成一个字符串；错误示例：`"stem_latex": "[\"第一段\", \"第二段\"]"`。
- 普通文本换行不要写成 \\。\\ 只允许出现在 aligned、cases、matrix、array 等 LaTeX 环境内部。
- 不要输出空字符串数组项。
- LaTeX 命令不能拆开，例如 \\frac、\\sqrt、\\leq、\\in 必须完整。
- 工具参数是 JSON 字符串；所有 LaTeX 反斜杠必须按 JSON 字符串规则正确转义。例如 LaTeX 的 \\because 必须在工具参数中写成 \\\\because，\\frac 必须写成 \\\\frac，不能写成会被 JSON 解释为控制字符的 \\b、\\f、\\t、\\n。
- <blank> 和 <choice_blank> 只能在数学环境外。
- 字段中不得输出 HTML 实体，例如 &gt;、&lt;、&amp;、&nbsp;。数学关系符号直接写为 <、>、\\leq、\\geq 等；普通文本也直接写真实字符。
- 唯一允许的 HTML 是原文表格需要保留时的单行 <table>...</table>；除此之外不得输出 HTML 标签或 HTML 实体。
...
- non_text_placeholders：表格和图片占位符映射；遇到 <imagexx>、<chartxx>、<figurexx> 时，必须用其中的 label 图片标号替代。
...
- 如果 A/B/C/D 选项出现在同一行、同一个 OCR 块或同一张裁剪图中，也必须拆分到对应键；例如图中依次出现 (A)、(B)、(C)、(D)，则分别填写 options_latex.A、options_latex.B、options_latex.C、options_latex.D，不能把四项都放入 A。
- 不得按行、列或版面左右分组填写 options_latex。两行两列选项仍按可见标签归位：第一项进 A，第二项进 B，第三项进 C，第四项进 D。
- 如果选项只有图片或图形，没有文字，应把对应图片标号写入 options_latex，例如 ["Q-V02-P01"] 或 ["M-V02-P01"]；不得写成 <image01>、<figure01> 或图片描述。图片归属仍由 Step4 处理。
...
- 如果答案 OCR 或答案/解析图只给出最终答案、答案表或答案列表，没有可见解答步骤、证明过程、计算过程、思路分析、解析、评析、归纳总结，则 analysis_latex 必须输出空数组 []。不得为了补全字段而自行推导、补写或生成解析。
...
- 图片内容不要 OCR 或描述进正文。
- <imagexx>、<chartxx>、<figurexx> 是临时占位符，不得原样输出。
- 如果临时占位符对应的 OCR 元数据有图片标号，则输出图片标号本身，例如 Q-V01-P01、A-V02-P01、M-V03-P01。
- 题干、答案、解析、评分标准中的图片位置写图片标号，不写图片描述；Step4 再决定其角色和归属。
- 图形选项也写图片标号到对应 options_latex。
...
def call_tool_calling(
    ...
    tool_schema = STEP3_IMAGE_ONLY_TOOL_SCHEMA if image_only else STEP3_TOOL_SCHEMA
    tool_description = (
        "提交从题面图和答案解析图整理出的单题题库记录。"
        if image_only
        else "提交一道数学试题的 Step3 结构化记录。"
    )
    tool_messages = [
        *messages,
        {
            "role": "user",
            "content": f"必须调用 {tool_name} 工具提交结果；不要在 message.content 输出 JSON、Markdown 或解释文字。",
        },
    ]
    body = {
        "model": model,
    ...
    try:
        ...
        if not tool_calls:
            raise RuntimeError(f"missing tool_calls: {json.dumps(message, ensure_ascii=False)[:1200]}")
        function = tool_calls[0].get("function") or {}
        if function.get("name") != tool_name:
            raise RuntimeError(f"unexpected tool name: {function.get('name')}")
        return function["arguments"]
    except (KeyError, IndexError, TypeError) as exc:
        raise RuntimeError(f"LLM API unexpected tool response: {json.dumps(payload, ensure_ascii=False)[:1200]}") from exc
...
def call_structured_output(
    ...
    if structured_output == "json_schema":
        return call_json_schema(messages, model=model, timeout=timeout, token=token, enable_thinking=enable_thinking)
    if structured_output == "tool_calling":
        return call_tool_calling(
            messages,
            model=model,
            timeout=timeout,
            token=token,
...
def content_warnings(record: Step3Record, compact: dict[str, Any]) -> list[str]:
    ...
    options_dump = record.options_latex.model_dump()
    if (
        record.question_type in {"single_choice", "multiple_choice"}
        and len(options_dump.get("A") or []) >= 2
        and not any(options_dump.get(label) for label in ("B", "C", "D"))
    ):
        warnings.append("choice options appear collapsed into options_latex.A; split visible A/B/C/D options into separate keys")
    nonempty_option_keys = [label for label in ("A", "B", "C", "D") if options_dump.get(label)]
    option_item_count = sum(len(options_dump.get(label) or []) for label in ("A", "B", "C", "D"))
    if record.question_type in {"single_choice", "multiple_choice"} and option_item_count >= 4 and len(nonempty_option_keys) < 4:
        warnings.append("choice options appear grouped by layout instead of A/B/C/D labels; split each visible option into its matching key")
...
def minimal_payload_for_model(payload: dict[str, Any], no_ocr_input: bool = False) -> dict[str, Any]:
    ...
    if no_ocr_input:
        return {
            "question_no": qno,
            "mode": mode,
            "boundary_hint": f"只整理第{qno}题。题面裁剪可能按上下文策略额外包含上一题末尾或下一题开头各一个标签，它们只用于判断边界；不要把相邻题内容写入第{qno}题。不要依赖 OCR 文本；题面和答案解析均以图片为准。",
...
def prepare_step3_answer_crop_images(payload: dict[str, Any], runs_root: Path, out_dir: Path) -> list[Path]:
    run_id = str(payload.get("run_id") or "")
    qno = int(payload["question_no"])
    labels = unique_str([str(label) for label in payload.get("source_answer_labels") or [] if str(label)])
    if not labels and str(payload.get("pipeline_mode") or "") == "mixed":
        surface_labels = set(question_surface_crop_labels(payload))
        labels = unique_str(
            [
                str(label)
                for label in payload.get("question_labels") or []
                if str(label) and str(label) not in surface_labels
            ]
        )
    if not labels:
        return []
    return crop_labels_to_images(
        run_id=run_id,
        qno=qno,
        labels=labels,
        run_block_dir=runs_root / f"{run_id}_raw_units",
        crop_dir=out_dir / "per_question" / "step3_answer_crops",
        filename_role="answer_surface",
        margin=0,
        split_shared_bboxes=False,
        asset_boxes_only=True,
    )
...
def build_step3_messages(
    compact: dict[str, Any],
    question_crop_images: list[Path],
    answer_crop_images: list[Path] | None = None,
) -> list[dict[str, Any]]:
    answer_crop_images = answer_crop_images or []
    if compact.get("input_mode") == "image_only_no_ocr":
        qno = int(compact["question_no"])
        content: list[dict[str, Any]] = [
            {"type": "text", "text": IMAGE_ONLY_USER_PROMPT_TEMPLATE.format(question_no=qno)}
        ]
        for index, image_path in enumerate(question_crop_images, start=1):
            content.append({"type": "text", "text": f"题面裁剪图 {index}："})
            content.append({"type": "image_url", "image_url": {"url": data_uri(image_path)}})
        for index, image_path in enumerate(answer_crop_images, start=1):
            content.append({"type": "text", "text": f"答案/解析裁剪图 {index}："})
            content.append({"type": "image_url", "image_url": {"url": data_uri(image_path)}})
        return [
            {"role": "system", "content": IMAGE_ONLY_SYSTEM_PROMPT},
            {"role": "user", "content": content},
        ]
    if not question_crop_images and not answer_crop_images:
        return [
    ...
    for index, image_path in enumerate(question_crop_images, start=1):
        ...
        content.append({"type": "image_url", "image_url": {"url": data_uri(image_path)}})
    for index, image_path in enumerate(answer_crop_images, start=1):
        content.append({"type": "text", "text": f"第{qno}题答案/解析裁剪图 {index}："})
        content.append({"type": "image_url", "image_url": {"url": data_uri(image_path)}})
...
def run_question(
    ...
    compact = minimal_payload_for_model(payload, no_ocr_input=no_ocr_input)
    ...
    try:
        ...
        answer_crop_images = prepare_step3_answer_crop_images(payload, runs_root, out_dir)
        ...
        messages = build_step3_messages(compact, question_crop_images, answer_crop_images)
        raw = call_structured_output(
            messages,
            model=model,
            timeout=timeout,
            token=token,
        ...
        parsed, structural_repairs = normalize_structural_fields(json.loads(raw))
...
def summarize(run_id: str, records: list[dict[str, Any]], results: list[dict[str, Any]], out_dir: Path, model: str) -> dict[str, Any]:
    ...
    structural_repair_counts: dict[str, int] = {}
    ...
    for result in results:
        ...
        for repair in result.get("structural_repairs") or []:
            if not isinstance(repair, dict):
                continue
            kind = str(repair.get("kind") or "unknown")
            field = str(repair.get("field") or "unknown")
            key = f"{kind}:{field}"
            structural_repair_counts[key] = structural_repair_counts.get(key, 0) + 1
            item = dict(repair)
            item["question_no"] = result.get("question_no")
            structural_repairs.append(item)
    summary.update(
        {
            "model": model,
            ...
            "structural_repair_counts": structural_repair_counts,
            "structural_repairs": sorted(
                structural_repairs,
                key=lambda item: (int(item.get("question_no") or 0), str(item.get("field") or "")),
            ),
            "results": [
...
def main() -> int:
    ...
    parser.add_argument("--structured-output", choices=["json_schema", "tool_calling"], default="json_schema")
    parser.add_argument("--no-ocr-input", action="store_true", default=False)
    ...
    summaries = [
        run_one_run(
            ...
            no_ocr_input=args.no_ocr_input,
        )
