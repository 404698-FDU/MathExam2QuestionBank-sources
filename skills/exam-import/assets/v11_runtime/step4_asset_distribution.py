...
VISUAL_ASSIGNMENT_TOOL_SCHEMA = {
    "type": "object",
    "required": ["assets", "risks"],
    "properties": {
        "assets": {
            "type": "array",
            "items": {
                "type": "object",
                "required": ["label", "question_no", "role", "option_label", "caption_labels", "caption_text", "confidence", "reason"],
                "properties": {
                    "label": {"type": "string"},
                    "question_no": {"type": "integer"},
                    "role": {"type": "string"},
                    "option_label": {"type": "string"},
                    "caption_labels": {"type": "array", "items": {"type": "string"}},
...
ANSWER_TABLE_TOOL_SCHEMA = {
    "type": "object",
    "required": ["tables", "risks"],
    "properties": {
        "tables": {
            "type": "array",
            "items": {
                "type": "object",
                "required": ["label", "role", "entries", "confidence", "reason"],
                "properties": {
                    "label": {"type": "string"},
                    "role": {"type": "string", "enum": ["answer_key_table", "rubric_table", "solution_table", "noise", "uncertain"]},
                    "entries": {
                        "type": "array",
                        "items": {
                            "type": "object",
                            "required": ["question_no", "answer_text", "confidence", "reason"],
                            "properties": {
                                "question_no": {"type": "integer"},
...
def build_visual_asset_assignment_messages(
    run_id: str,
    mode: str,
    ...
    if structured_output == "tool_calling":
        output_rule = (
            "输出约束：\n"
            "- 必须调用工具 submit_step4_visual_assets 提交结果。\n"
            "- 普通回复正文必须为空；不得在 message.content 中写 JSON、Markdown、代码块、解释或分析过程。\n"
            "- 工具参数必须包含 assets 和 risks。\n\n"
            "工具参数格式示例：\n"
        )
        final_rule = "\n\n现在调用工具 submit_step4_visual_assets。普通正文保持为空。"
        system_prompt = (
            "你只做中文数学试卷图像、图表、表格资产归属。"
            "本次任务必须通过调用工具 submit_step4_visual_assets 完成，不得在普通回复正文中输出结果。"
        )
    else:
        output_rule = "只返回严格 JSON，格式如下：\n"
        final_rule = ""
        system_prompt = "你只做中文数学试卷图像、图表、表格资产归属，返回严格 JSON。"
    instruction = (
        "/no_think\n"
        ...
        + output_rule
        + '{"assets":[{"label":"Q-V08-P01","question_no":17,"role":"stem_figure","option_label":"",'
...
def assign_visual_assets_with_llm(
    ...
    def call_page(context: dict[str, Any]) -> dict[str, Any]:
        messages = build_visual_asset_assignment_messages(run_id, pipeline_mode, context, structured_output)
        ...
        parsed, raw, elapsed = call_chat_structured_json(
            ...
            tool_name="submit_step4_visual_assets",
            tool_description="提交 Step4 单页视觉资产归属结果。",
            tool_schema=VISUAL_ASSIGNMENT_TOOL_SCHEMA,
...
def build_answer_table_extraction_messages(
    run_id: str,
    ...
    if structured_output == "tool_calling":
        output_rule = (
            "输出约束：\n"
            "- 必须调用工具 submit_step4_answer_tables 提交结果。\n"
            "- 普通回复正文必须为空；不得在 message.content 中写 JSON、Markdown、代码块、解释或分析过程。\n"
            "- 工具参数必须包含 tables 和 risks。\n\n"
            "工具参数格式示例：\n"
        )
        final_rule = "\n\n现在调用工具 submit_step4_answer_tables。普通正文保持为空。"
        system_prompt = (
            "你只做中文数学试卷答案页表格分类和答案条目抽取。"
            "本次任务必须通过调用工具 submit_step4_answer_tables 完成，不得在普通回复正文中输出结果。"
        )
    else:
        output_rule = "只返回严格 JSON，格式如下：\n"
        final_rule = ""
        system_prompt = "你只做中文数学试卷答案页表格分类和答案条目抽取，返回严格 JSON。"
    instruction = (
        "/no_think\n"
        ...
        "- 如果表格把题号映射到选择字母或最终答案，role 使用 answer_key_table。\n"
        "- 如果表格是评分标准、解答过程表、版面噪声，或不能作为最终答案，entries 为空。\n"
        "- 选择题字母答案必须写成 LaTeX 行内格式，例如 $A$。\n"
        "- question_no 应来自表格可见题号，通常应位于 candidate_qnos 中。\n\n"
        + output_rule
        + '{"tables":[{"label":"A-V01-B22","role":"answer_key_table|rubric_table|solution_table|noise|uncertain",'
        '"entries":[{"question_no":13,"answer_text":"$B$","confidence":0.0,"reason":"简短中文理由"}],'
        '"confidence":0.0,"reason":"简短中文理由"}],'
        '"risks":[{"label":"A-V01-B22","severity":"info|warning|error","reason":"简短中文风险"}]}\n\n'
        "紧凑上下文：\n"
        + json.dumps(compact, ensure_ascii=False, indent=2)
        + final_rule
    )
    ...
    if not image_path.is_absolute():
        image_path = WORKTREE_ROOT / image_path
    if image_path.exists():
        content.append({"type": "text", "text": f"{context.get('stream')} 第 {context.get('page')} 页标注图："})
        content.append(
            {
                "type": "image_url",
                "image_url": {
...
def extract_answer_tables_with_llm(
    ...
    def call_page(context: dict[str, Any]) -> dict[str, Any]:
        messages = build_answer_table_extraction_messages(run_id, context, structured_output)
        request_model = str(context.get("_worker_model") or model)
        parsed, raw, elapsed = call_chat_structured_json(
            messages,
            model=request_model,
            timeout=timeout,
            structured_output=structured_output,
            tool_name="submit_step4_answer_tables",
            tool_description="提交 Step4 答案表格分类与答案条目抽取结果。",
            tool_schema=ANSWER_TABLE_TOOL_SCHEMA,
            enable_thinking=enable_thinking,
        )
